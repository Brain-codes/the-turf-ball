...
window.PROGRESS = {
  ...
  updated: "17 Sep 2026",
  currentlyDoing: "Smoother landing page, a matching look for the other public pages, and a spam-protected contact form. The 3D now moves exactly with your scroll. The contact form is live on the server side and tested. The page changes are checked in the preview and go live when deployed.",
  ...
  phases: [
    ...
    },
    {
      ...
      tasks: [
        ...
        { name: "Anyone can be given a clean sheet", plain: "You record these yourself now, per set, as they happen. Tap 'Clean sheet', tap whoever was at the post — anyone can go in goal, so it is never limited to a nominated keeper. Keep five sets clean and that is five clean sheets on the night, all counting towards points and the Golden Glove.", tech: "The automatic award at full time is GONE, not just bypassed. It read match_players.is_goalkeeper, which no screen has ever set, so it credited nobody under the default policy and every player on the pitch under 'whole_side' — which would have flooded the hand-recorded counts. finishMatch and resumeMatch no longer read, write or delete clean_sheet events at all. No uniqueness constraint either: the app does not model sets, so taps simply stack.", status: "done" },
    ...
    {
      id: 22,
      name: "Sessions pause and end themselves again (bug you spotted)",
      plain: "You asked why the 6 September session was still live. The part of the app that runs every five minutes to create, start, pause and tidy up sessions had been crashing on every single run for weeks, so none of that was happening. It's fixed and confirmed working on your live project.",
      status: "done",
      starred: true,
      tasks: [
        { name: "The five-minute job works again", plain: "Its first successful run (17 Sep, 12:25) immediately created your next session — Sunday 20 September at 7pm — which is what it's supposed to do a week ahead.", tech: "materialize_and_flag_sessions() read slot.duration_minutes, a column upcoming_slots() never returns, so it failed on its first statement every run (3,456 consecutive failures in the retained history). Duration is now looked up from session_slots. Migration 20260917000000.", status: "done" },
        { name: "Paused after 30 quiet minutes", plain: "If nothing is recorded for 30 minutes, the session pauses. Nothing is lost — resume it and carry on where you left off.", tech: "Pause threshold moved from 20 to 30 minutes. App wording updated on the match day, session and home screens.", status: "done" },
        { name: "Ended after 24 quiet hours", plain: "If a session sits untouched for a whole day, the app ends it for you — exactly as if you'd pressed End session — so it can't hold the month open or leave a clock running. You still get the usual 5 hours afterwards to fix anything.", tech: "New step ends live or paused sessions with no activity for 24h: completes unfinished matches, sets status completed, ended_at now(), ended_reason 'inactivity', and recomputes the month (never a closed one).", status: "done" },
        { name: "A duplicate-session trap, closed before it could bite", plain: "A session set for just after midnight would have been created again and again, every five minutes. It never happened only because the job was broken. Fixed now, before anyone sets a late slot.", tech: "session_date is now the group's local date, matching the already_scheduled check in upcoming_slots(). Tested with a 00:30 Lagos slot across three consecutive runs: one session.", status: "done" },
        { name: "New 3D landing page", plain: "The home page is now a floodlit pitch at night. As you scroll, the camera moves around the pitch and the ball travels into the net, while each section explains a feature: match day, points, awards, sharing and the automatic weekly setup. There's also a questions section, a floating menu and a proper footer with 'Powered by Anonimos Brain'.", tech: "Three.js scene (src/features/landing/scene.ts), loaded on demand; GSAP ScrollTrigger drives camera progress and section reveals; honours reduced-motion; falls back to a gradient without WebGL. New PublicNavbar (floating pill) and SiteFooter.", status: "done" },
        { name: "Search engine setup (SEO) on every page", plain: "Each page now tells Google what it is. Public pages can be found; private pages (your dashboard, settings, sign-in helpers) are hidden from search. Shared team and player links show the right name and description in WhatsApp. Added a sitemap, a robots file, app icons and a share image.", tech: "src/lib/seo.ts (useSeo + RouteSeo), JSON-LD (WebSite, SoftwareApplication, FAQPage, SportsTeam, Person), api/share.ts injects meta for /t/* at the edge, api/sitemap.ts lists public teams, robots.txt, site.webmanifest, og-image.png.", status: "done" },
        { name: "Landing page follows your scroll exactly, and runs lighter", plain: "The 3D pitch used to ease towards your scroll position, so it trailed behind. It now moves with you frame for frame. It's also much cheaper to draw: the floodlight lighting is painted onto the pitch instead of calculated live, and the frosted-glass panels, which were very expensive over moving 3D, are now solid.", tech: "scene.ts reads scrollY inside the rAF frame with no lerp (only mouse sway is eased); 4 SpotLights replaced by baked light pools + hemisphere light; DPR capped at 1.5 / 1.25; dust halved; backdrop-blur removed from landing cards.", status: "done" },
        { name: "Matching look for the other public pages", plain: "Tables, head-to-head, team pages, player pages and live scores now share the landing page's style, in a lighter form: a slowly drifting pitch background, bigger headings, cards that ease in as you scroll, the floating menu and the new footer. No 3D, so they stay quick on mobile data.", tech: "PublicLayout + PublicHero (src/components/layout/PublicLayout.tsx): CSS-only backdrop, GSAP reveal driven by IntersectionObserver + MutationObserver (handles data that loads later), failsafe so nothing stays hidden, scroll reset on page change.", status: "done" },
        { name: "Contact form, protected from spam", plain: "A new Contact page lets anyone send a question, idea, bug report or partnership request. Bots are blocked several ways: a hidden trap field, a server check that rejects forms sent too fast, limits on how many messages one person or email can send, a daily cap on total messages, duplicates dropped, and no web code allowed. Messages land in a private table only you can read, in Supabase under contact_messages.", tech: "Migration 20260917120000 (contact_messages, RLS deny-all, anon/authenticated revoked). Edge Function `contact`: GET token (HMAC-signed issue time), POST submit (honeypot, 3 s to 24 h token age, 3/hour + 8/day per salted IP hash, 3/day per email, 300/day global, 2-link cap, HTML rejected, 16 KB body cap). Probed live with curl: all rejections behave; anon REST read denied. Test rows deleted.", status: "done" }
      ]
    }
  ],
  ...
  log: [
    ...
    { time: "16 Sep 2026", text: "YOU ASKED: head to head for two players. Built: pick any two players, from two completely different teams, and see them side by side: appearances, goals, assists, per-game averages, clean sheets, saves, penalty saves, awards, and cards (fewer is better there). The better number on each line lights up, with a 'categories won' score at the top. Anyone can use it at /h2h (public teams only, new H2H link in the top bar). Signed in, it's at /app/h2h and also includes your own team even if it isn't public. Open it from the Table page, or the Compare button on any player's profile. The link can be shared and opens the same two players.", kind: "done" },
    { time: "16 Sep 2026", text: "TESTED: the server changes are live. Compared two players from two different 'Sunday Ballers' teams, checked that picking the same player twice is refused, a made-up player is turned away, and the signed-in version refuses anyone not logged in. Clicked through the public page in the browser on desktop and phone size, including reopening a shared link. The signed-in version hasn't been clicked through yet.", kind: "done" },
    { time: "17 Sep 2026", text: "YOU ASKED: a designed head-to-head image to download and share, with the link. Built: three buttons above every comparison. SHARE (on phones) opens the share sheet with the image plus a caption like 'Adeleke vs Agada: 5-2 on The Turf Ball. Who's better? <link>'. The caption is also copied, because WhatsApp sometimes drops text sent with an image, so you can just paste it. DOWNLOAD IMAGE saves the picture. COPY LINK copies the caption and link. On a computer, Share downloads the image and copies the caption. The image is a 1080x1350 dark pitch-style card with both photos, a glowing ring on the winner, the score, 8 stat lines with bars, and the site address at the bottom. The link always opens the public page. If one of the players is on a private team, the app warns you the link won't open for other people.", kind: "done" },
