-- ============================================================================
-- A cancelled session counts for nothing.
--
-- Found while sorting out a 30 August session that was never actually played.
-- Ending it from the app marked it 'completed' and recomputed August — a month
-- that had already been closed — so everyone who had been picked into teams
-- that evening gained an August appearance out of nowhere.
--
-- Marking the session 'cancelled' did not help, because nothing looked at the
-- session's status. Appearances were counted from the MATCH status, punctuality
-- from attendance rows, and events from their own period stamp. A cancelled
-- session still paid out on all three.
--
-- Now: appearances, points, punctuality and every recorded event belonging to a
-- cancelled session are ignored. Cancelling a session and recomputing its month
-- puts the numbers back exactly as they were.
--
-- Based on the version in 20260906000100_penalty_saves_and_clean_sheets.sql;
-- the only changes are the three cancelled-session filters.
-- ============================================================================
